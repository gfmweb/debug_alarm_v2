			...
			<template v-if="formLoginData!==null">
				<div class="card">
					...
					<div class="card-body">
						...
						<form id="form" :action="formLoginData.action[formOperationIndex]" :method="formLoginData.method" v-on:submit.prevent="calculateForm">
		...
		methods:{
			...
			calculateForm(){
				...
				for(let i = 0; i < fieldsCount; i++) {
					const fname = forma.elements[i].name
					const fval = forma.elements[i].value
					FormDat.push({fname: fval})
				}
				axios.post(urI,FormDat).then(res=>{
					if(res.data.errors == null){
						if(self.formOperationIndex+1 < self.formLoginData.form_fields.length){
							...
							console.log('Must redirect to another page')
						}
					}
					else{
						self.formErrors = res.data.errors
					}
				})
				
			},
			makeFormRequestCheckCode(){
